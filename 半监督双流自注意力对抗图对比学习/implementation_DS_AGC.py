...
sys_path = os.path.abspath("/sdj/cyq/semi_supervised")
def setup_seed(seed):
...
def train_GCN_contrast(subject_id, parameter, net_params, source_labeled_loaders, source_unlabeled_loaders, target_loader):
    ...
    # 替换 RMSprop 优化器为 AdamW 优化器
    optimizer = AdamW([
        {'params': model.parameters(), 'lr': parameter['init_lr'], 'weight_decay': parameter['weight_decay']},
        {'params': awl.parameters(), 'weight_decay': 0}
    ])
...
def get_dataset(test_id, parameter):
    path=[]
    path.append(sys_path+'/de_feature_SEED/feature_for_net_session'+str(1)+'_LDS_de')
    path.append(sys_path+'/de_feature_SEED/feature_for_net_session'+str(2)+'_LDS_de')
    path.append(sys_path+'/de_feature_SEED/feature_for_net_session'+str(3)+'_LDS_de')
    ## our label:0 negative, label:1 :neural,label:2:positive, seed original label: -1,0,1, our label= seed label+1
    min_max_scaler = preprocessing.MinMaxScaler(feature_range = (-1, 1))
...
def main(parameter, net_params):
   # data preparation
    ...
    if net_params["category_number"] == 3:
        ...
        sub_num = 16
    print('BS: {}, epoch: {}，NUM_U：{}'.format(parameter['batch_size'], parameter['epochs'], parameter['num_of_U']))
    # store the results
    csub = []
    ...
    for subject_id in range(sub_num):
        if net_params['category_number'] == 3:
            target_set, source_set_labeled, source_set_unlabeled = get_dataset(subject_id, parameter)
            Dataset_name = 'SEED'
        elif net_params['category_number'] == 4:
...
parameter = {'epochs':100, 'batch_size':48, 'init_lr':1e-3, 'weight_decay':1e-5, "semi": 1, 'threshold': 30, 'num_of_U':7, 'drop_rate':0.8,
             'GLalpha': 0.01, 'node_feature_hidden1': 5,'linearsize': 128, 'category_number': 4, 'DEVICE': 'cuda:0', 'K': 3, 'num_of_vertices': 62, 'num_of_features': 5,
